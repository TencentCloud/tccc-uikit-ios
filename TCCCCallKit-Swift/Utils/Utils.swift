...
@objc
public class DisplayUtils: NSObject {
    
    @objc
    public func maskTelephoneNumber(_ telNumber: String) -> String {
        if telNumber.isEmpty || telNumber.count <= 6 {
            return telNumber
        }
        
        var sb = String()
        sb.append(String(telNumber.prefix(3)))
        
        for _ in 3..<(telNumber.count - 3) {
            sb.append("*")
        }
        
        sb.append(String(telNumber.suffix(3)))
